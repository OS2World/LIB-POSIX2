...
static void c_output __P((const char *, const char *, int, char *));
static void h_output __P((const char *, const char *, int, char *));
static void s_output __P((int, char **, const char *, const char *, int, char *, int, int));
static void l_output __P((const char *, const char *, int, char *));
static void t_output __P((const char *, const char *, int, char *));
static void svc_output __P((const char *, const char *, int, char *));
static void clnt_output __P((const char *, const char *, int, char *));
static int do_registers __P((int, char *const*));
static void addarg __P((const char *));
static void putarg __P((int, const char *));
static void clear_args __P((void));
static void checkfiles __P((const char *, char *));
...
{
  struct stat buf;
  static char CPP_buf[MAXPATHLEN];

  if( stat(CPP, &buf) < 0 )  {	/* SVR4 or explicit cpp does not exist */
    if (cppDefined) {
      fprintf( stderr, "rpcgen: Cannot find C preprocessor: <%s>!\n", CPP );
      crash();
    } else {			/* try the other one */
      if (_path(CPP_buf,P2_CPP)) {
        fprintf( stderr, "rpcgen: Cannot find any C preprocessor (cpp)\n" );
        crash();
      }
      CPP = CPP_buf;
    }
	...
	int netflag;
{
	...
	write_most((char*)infile, netflag, nomain);
	if (!nomain) {

    ...
    char   *pathname;
{
	...
	(void) sprintf(pathname,
			"pmake: getwd(): Cannot stat \"/\" (%s)", strerror(errno));
	return (NULL);
    }
	...
	if (stat(nextpathptr, &st_dotdot) == -1) {
	    (void) sprintf(pathname,
			    "pmake: getwd(): Cannot stat directory \"%s\" (%s)",
			    nextpathptr, strerror(errno));
	    return (NULL);
	}
	if ((dp = opendir(nextpathptr)) == NULL) {
	    (void) sprintf(pathname,
			    "pmake: getwd(): Cannot open directory \"%s\" (%s)",
			    nextpathptr, strerror(errno));
	    return (NULL);
	}
	...
	if (DEV_DEV_COMPARE(st_dotdot.st_dev, st_cur.st_dev)) {
		...
		if (lstat(nextpathptr, &st_next) == -1) {
		    (void) sprintf(pathname, "pmake: getwd(): Cannot stat \"%s\" (%s)",
	...
	if (d == NULL) {
	    (void) sprintf(pathname, "pmake: getwd(): Cannot find \".\" in \"..\"");
	    (void) closedir(dp);
	    return (NULL);
	}
	st_cur = st_dotdot;

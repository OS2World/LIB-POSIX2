/*	$Id: inp.c,v 1.2 2000/08/21 13:37:23 amai Exp $	*/
...
#define try(f, a1, a2, a3) (Snprintf(s, sizeof lbuf - 20, f, a1, a2, a3), stat(s, &cstat) == 0)
    ...
    i_womp = malloc((i_size+2));	/* lint says this may alloc less than */
					/* i_size, but that's okay, I think. */
    if (i_womp == NULL)
	return FALSE;
    if ( NULL == (in_fp = fopen(filename, "r")) )
	pfatal2("can't open file %s", filename);
	{size_t i = fread(i_womp, i_size, 1, in_fp);
    if (ferror(in_fp) || i > i_size) {
	fclose(in_fp);	/* probably means i_size > 15 or 16 bits worth */
...
plan_b(const char *filename)
{
    ...
    Reg1 int i = 0;
    Reg2 int maxlen = 1;
    Reg4 bool found_revision = (revision == NULL);

...
int getrec(char **pbuf, size_t *pbufsize, int isrecord)	/* get next input record */
...
void fldbld(void)	/* create fields from current record */
{
	...
	if (n > fieldssize) {
		...
		if ((fields = (char *) malloc(n+1)) == NULL)
			ERROR "out of space for fields in fldbld %ld", n FATAL;
		fieldssize = n;
	}
	fr = fields;
...
int refldbld(char *rec, char *fs)	/* build fields from reg expr in FS */
{
	...
	if (n > fieldssize) {
		xfree(fields);
		if ((fields = (char *) malloc(n+1)) == NULL)
			ERROR "out of space for fields in refldbld %ld", n FATAL;
		fieldssize = n;
	}
	fr = fields;
	*fr = '\0';

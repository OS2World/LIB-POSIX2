...
readtcp(
	...
	register int len)
{
	register SVCXPRT *xprt = (SVCXPRT *)cad;
	register int sock = xprt->xp_sock;
	struct timeval start, delta;
	struct timeval tmp1, tmp2;
	fd_set *fds = NULL;
	size_t prevbytes = 0, bytes;
	...
	do {
		...
		if (fds == NULL)
			goto fatal_err;
		__builtin_memcpy(fds, __svc_fdset, bytes);
	...
	u_int recvsize)
{
	...
	static bool_t
rendezvous_request(register SVCXPRT *xprt, struct rpc_msg *msg)
{
	int sock;

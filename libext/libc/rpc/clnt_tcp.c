...
readtcp(
	...
	register int len)
{
	register struct ct_data *ct = (struct ct_data *)cat;
	fd_set *fds, readfds;
	...
	if (ct->ct_sock+1 > FD_SETSIZE) {
		size_t bytes = howmany(ct->ct_sock+1, NFDBITS) * sizeof(fd_mask);
...
{
	register int i, cnt;
	struct ct_data *ct = (struct ct_data *)cat;
	...
	u_int recvsz;
{
	 ...
	 */
	(void)_gettimeofday(&now, (struct timezone *)0);
	call_msg.rm_xid = _arc4random();

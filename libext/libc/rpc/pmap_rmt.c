	...
	int sock)  /* any valid socket will do */
{
	...
	addrs = (struct in_addr *)malloc((inbuflen / sizeof *sadr_in) * sizeof sadr_in);
	if (addrs == NULL) {
		*addrsp = NULL;
		free(inbuf);
		return (0);
...
#define size(p)	max((p).sa_len, sizeof(p))
	...
	for (cp = inbuf; cp < cplim;
		...
		if ((ifreq.ifr_flags & IFF_BROADCAST) &&
			...
			sadr_in = (struct sockaddr_in *)&ifr->ifr_addr;
			if (ioctl(sock, SIOCGIFBRDADDR, (char *)&ifreq) < 0) {
				addrs[i++] =
				    inet_makeaddr(inet_netof(sadr_in->sin_addr),
				    INADDR_ANY);
			} else {
	...
	resultproc_t	eachresult;	/* call with each result obtained */
{
	...
	if (sock+1 > FD_SETSIZE) {
		size_t bytes = howmany(sock+1, NFDBITS) * sizeof(fd_mask);
	...
	(void)_gettimeofday(&t, (struct timezone *)0);
	msg.rm_xid = xid = _arc4random();
	t.tv_usec = 0;
	...
	recv_again:
		...
		msg.acpted_rply.ar_results.proc = (resultproc_t)xdr_rmtcallres;
	...
	try_again:
		...
		xdrs->x_op = XDR_FREE;
		msg.acpted_rply.ar_results.proc = (resultproc_t)xdr_void;

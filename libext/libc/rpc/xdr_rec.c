	...
	register int len)
{
	...
	while (len > 0) {
		...
		__builtin_memcpy(addr, rstrm->in_finger, (size_t)current);
		rstrm->in_finger += current;
		addr += current;
		len -= current;
	}
...
{
	register RECSTREAM *rstrm = (RECSTREAM *)(xdrs->x_private);
	register u_long current;

	while (len > 0) {
		current = (u_long)rstrm->out_boundry -
		    (u_long)rstrm->out_finger;
		current = (len < current) ? len : current;
		__builtin_memcpy(rstrm->out_finger, addr, current);
...
xdrrec_getlong(XDR *xdrs, long *lp)
{
	...
	if ((rstrm->fbtbc >= (int)sizeof(int32_t)) &&
		...
		*lp = (long)ntohl((u_int32_t)(*buflp));
		...
		if (! xdrrec_getbytes(xdrs, (caddr_t)&mylong, (int)sizeof(int32_t)))
			return (FALSE);
...
xdrrec_getpos(register XDR *xdrs)
{
	...
	pos = _lseek((int)(long)rstrm->tcp_handle, (off_t)0, 1);
	if (pos != -1)
		switch (xdrs->x_op) {

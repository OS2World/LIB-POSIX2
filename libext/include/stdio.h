/* libext+gcc: stdio.h 
 * $Header: /cvsroot/posix2/posix2/libext/include/stdio.h,v 1.3 2000/11/25 16:33:35 arnd Exp $
 */
...
int fileno(FILE*);
extern __inline__ int fileno(FILE*__s) { return __s->_handle; }
#endif
...
extern int _popenRW(__const__ char*__const__*,int*,int);
extern char*_fgetln(FILE*,size_t*);
extern char*_fparseln(FILE*,size_t*,size_t*,__const__ char[3],int);
#endif
...
int putw(int, FILE*);
void setbuffer(register FILE*, char*, size_t);
#define setbuffer(F,b,s) 	((void)setvbuf(F,b,b?_IOFBF:_IONBF,s))
int setlinebuf(FILE*);
#define setlinebuf(F) 	(setvbuf(F,(char*)NULL,_IOLBF,(size_t)0))
...
/* 4.4 BSD */
extern char*fgetln(FILE*,size_t*);			/* Get an input line. */
extern char*fparseln(FILE*,size_t*,size_t*,__const__ char[3],int);

#if !(!__GNUC__ || __GNUC__ < 2 || (__GNUC__ == 2 && __GNUC_MINOR__ <= 7))
extern __inline__ char*fgetln(FILE*__f,size_t*__s) {return _fgetln(__f,__s);}
extern __inline__  char *fparseln(FILE*__f,size_t*__s,size_t*__t,
	__const__ char __b[3],int __i){return _fparseln(__f,__s,__t,__b,__i);}
#endif
